function [precision, sensitivity, accuracy, recall, F1] = PerformanceEvaluationWindow(TP, FN, FP)
    ...
    % accuracy = (pixelTP+pixelTN) / (pixelTP+pixelFP+pixelFN+pixelTN);
    % Specificity = pixelTN / (pixelTN+pixelFP);
    recall = TP / (TP+FN);
    F1 = 2*(TP)/(2*TP+FN+FP);
end